...
void update_IO() {
  ...
  if (bitRead(Car.Doors, 6) && !Car.ign && curHUState) {
    //Door open, ign off, HU on.
    SerialBT.end();
    digitalWrite(IO_HUenable, 0);
  }
  if (Car.ign) {
    SerialBT.begin("PSA_CAN"); //Bluetooth device name
    digitalWrite(IO_HUenable, 1);
  }
}
  ...
  if (xQueueReceive(CAN_cfg.rx_queue, &canMsgRcv, 3 * portTICK_PERIOD_MS) == pdTRUE) {
    ...
    switch (canMsgRcv.MsgID) {
        ...
        case 0x0F6: //0x0F6 bsi TEMP COOLANT ?EXT? reverse gear ignition odometer
          ...
          if (Car.ign != bitRead(canMsgRcv.data.u8[0], 3)) {
            Car.ign = bitRead(canMsgRcv.data.u8[0], 3);
            Car.H_UPD_IO = true;
          }

          if (Car.rear != bitRead(canMsgRcv.data.u8[7], 7)) {
        ...
        case 0x1A1: //0x1A1 Informational message
          if (Car.INF_MSG != canMsgRcv.data.u8[1]) {
        ...
        default:
          if (canDump) { //ID,Extended,Bus,LEN,D1,D2,D3,D4,D5,D6,D7,D8
            SerialBT.print(String(canMsgRcv.MsgID, HEX));
            SerialBT.print(",false,0," + canMsgRcv.FIR.B.DLC);
            for (int x=0; x < canMsgRcv.FIR.B.DLC; x++) SerialBT.print("," + String(canMsgRcv.data.u8[x], HEX));
            SerialBT.println("");
          }
          break;             
        }
